#!/usr/bin/env python3
"""
Krea2 (NextDiT) layer distribution analyzer for HSWQ NVFP4 convert.
...
Produces per-layer statistics and:
  ...
  - derive_nvfp4_autonomous_tunables()
      auto analysis → auto-optimal for hswq_convert_nvfp4_krea2.py
...
def classify_layer(name: str) -> str:
    """Krea2 SingleStreamDiT Linear keys (with or without trailing .weight)."""
    ...
    for pfx in ("model.diffusion_model.", "diffusion_model.", "model."):
        if base.startswith(pfx):
            ...
            break
    if (
        base.endswith(".attn.wq")
        or base.endswith(".attn.wk")
        or base.endswith(".attn.wv")
    ):
        return "qkv"
    if base.endswith(".attn.wo"):
        return "toout"
    if base.endswith(".mlp.down"):
        return "ff2"
    if ".mlp.gate" in base or ".mlp.up" in base or base.endswith(".mlp.gate") or base.endswith(".mlp.up"):
        return "ff0"
    return "other"
...
def derive_veto_tunables_nvfp4(profile: Dict[str, Any]) -> Dict[str, Any]:
    """NVFP4 VETO + V4-histogram link for Krea2 NextDiT.
...
def _mad_continuous_fences_from_positives(
    ...
    *,
    ...
    Philosophy §1 / §14: auto analysis → infinite-branch auto-optimal.
      ...
      soft       = THIS below-floor Soft-band occupancy (collapse-shaped)
      P99        = tip / severity reference only — never a VETO floor
      collapse   = tip-heaviness fingerprint (shapes Soft soft)

    Forbidden soft formulas (already destroyed THIS quality):
...
def _derive_nvfp4_attn_mad_tunables(
    ...
    abs_max/std (outlier_ratio) misses heavy-tailed attn on some Krea2 checkpoints.
...
def nvfp4_fp16_budget_analyze_severity(
    *,
    ...
    # Krea2 / NextDiT: `.attn.wq/.wk/.wv` and `.attn.wo` (structure suffix).
...
# Krea2 / NextDiT structure endings only — never a frozen priority recipe.
...
# Owner hard ceiling NVFP4_FP16_BUDGET_MB_HARD (Krea2 convert: 3000 MiB) is
# NOT a thinking-stop recipe: auto knobs fill inside that frame and must
# never exceed it.
...
# Default = Krea2 NVFP4 convert budget ceiling (3000 MiB).
# Module attribute may be set before derive / assert; ranking / fill unchanged.
NVFP4_FP16_BUDGET_MB_HARD = 3000.0
...
def derive_nvfp4_autonomous_tunables(
    ...
    *,
    ...
    (3000 MiB for Krea2 NVFP4 convert).
...
def _is_unet_weight_key(name: str) -> bool:
    """Keep Krea2 DiT weights; drop VAE / text encoders if present."""
    if name.startswith(("conditioner.", "first_stage_model.", "text_encoder")):
        return False
    if name.startswith(("text_encoders.", "vae.")):
        return False
    if "vae." in name:
        return False
    # Prefer diffusion_model / DiT block keys when present.
    if "diffusion_model" in name or name.startswith("blocks.") or ".attn." in name:
        return True
    if name.startswith(("first.", "last.", "tmlp.", "tproj.", "txtmlp.", "txtfusion.")):
        return True
    return True
...
def measure_v4_nvfp4_mse_at_absmax(
    ...
    *,
    ...
    importance_by_layer MUST come from DualMonitor after the Krea2 r32
...
def generate_model_profile(input_path: str, output_path: str) -> Dict[str, Any]:
    ...
    Used by hswq_convert_nvfp4_krea2.py auto analyze path.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze Krea2 NextDiT weight distribution for HSWQ NVFP4"
    )
    parser.add_argument("safetensors", nargs="?", help="Path to Krea2 safetensors (positional)")
