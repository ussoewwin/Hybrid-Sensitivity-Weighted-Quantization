#!/usr/bin/env python3
"""
Krea2 DiT layer distribution analyzer for HSWQ INT8 (ConvRot).

Produces per-layer statistics and:
  - derive_veto_tunables_int8()  → INT8 / quantize_krea2_hswq_int8_v1.0
    (hard VETO fences + mse_* that drive V4 MSE-guided VETO; pack search_low=1.0)

FP16 protect / DualMonitor / int8_fp16_budget_* / V4 INT8 measure logic is the
same SDXL-INT8 structure (do not gut). Only layer key patterns are Krea2 DiT.
Pack / load / calib authority for the converter: native_convert_int8_krea2.py
...
def classify_layer(name: str) -> str:
    ...
    if (
        ".mlp.gate" in base
        or ".mlp.up" in base
        or base.endswith(".mlp.gate")
        or base.endswith(".mlp.up")
    ):
        return "ff0"
    return "other"
...
def int8_fp16_budget_analyze_severity(
    *,
    ...
    # Krea2: `.attn.wq/.wk/.wv` and `.attn.wo` (structure suffix).
...
# Krea2 SingleStreamDiT structure endings only — never a frozen priority recipe.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze Krea2 DiT weight distribution (HSWQ INT8 / ConvRot)"
    )
    parser.add_argument(
        "safetensors",
        nargs="?",
        help="Path to Krea2 DiT safetensors (positional)",
    )
    parser.add_argument(
        "--input",
        "-i",
        dest="input_path",
        help="Input safetensors (quantize CLI compat)",
    )
