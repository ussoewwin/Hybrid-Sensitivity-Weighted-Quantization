#!/usr/bin/env python3
"""
Z-Image / ZIT (NextDiT) layer distribution analyzer for HSWQ ConvRot NVFP4
+ INT8-protect ranking (quantize_zi_hswq_convrot_nvfp4_v1.0.py).

Produces per-layer statistics and:
  - derive_veto_tunables_nvfp4()
      hard VETO fences + mse_* for V4 pack-MSE-guided ranking
      (Linear=NVFP4; pack search_low=1.0)
...
NextDiT structure suffixes (.attention.qkv / .feed_forward.w* / adaLN) included.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze Z-Image/ZIT weight distribution for HSWQ ConvRot NVFP4 "
            "+ INT8-protect ranking"
        )
    )
    parser.add_argument(
        "safetensors",
        nargs="?",
        help="Path to Z-Image/ZIT safetensors (positional)",
    )
    parser.add_argument(
        "--input",
        "-i",
