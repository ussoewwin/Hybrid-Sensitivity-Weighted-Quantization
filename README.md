...
**Technical details (Z Image reverse method / trajectory-sensitivity impact ranking):** [md/diag_impact_trajectory_sensitivity_technical_guide.md](md/diag_impact_trajectory_sensitivity_technical_guide.md) — per-layer impact measured as the relative divergence of a fixed seed-locked 4-step denoising trajectory with one layer's NVFP4 reconstruction error injected; includes the full mathematical treatment of error interaction (Taylor expansion of the sampling map, cross terms, error cancellation), nonlinear amplification (Lyapunov-style growth), the low-error additivity regime where single-layer ranking is valid, marginal effects, and Shapley-style attribution — why static saliency (histogram MSE / cosine / SVD) cannot predict joint quantization error.
