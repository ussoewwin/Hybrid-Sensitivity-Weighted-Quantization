...
- **INT8 (Z Image V1.0):** Same DualMonitor + V4 / r0 FP16-protect design under a **700 MiB** budget (tensorwise pack; ConvRot is the SDXL V3.1 path).
...
**SDXL models (ConvRot INT8):** [Hugging Face — Hybrid-Sensitivity-Weighted-Quantization-SDXL-INT8](https://huggingface.co/ussoewwin/Hybrid-Sensitivity-Weighted-Quantization-SDXL-INT8)
...
| Feature | FP8 V1: Standard Compatible | FP8 V2: High Performance Scaled | ConvRot INT8 (SDXL V3.1) / INT8 (Z Image V1.0) |
| :--- | :--- | :--- | :--- |
...
| **Mechanism** | Optimal clipping (smart clipping) | Full-range scaling (dynamic scaling) | Absmax + DualMonitor / V4 FP16 protect (r0); **SDXL V3.1:** then FULL ConvRot on Linear/Conv2d remainder |
...
| **Use case** | Distribution, general users | Unavailable until a dedicated loader exists | SDXL ConvRot INT8 / Z Image INT8 distribution / kitchen loaders |
...
1. **Dual Monitor System** — During calibration, two metrics are collected:
   - **Sensitivity** (output variance): layers that hurt image quality most if corrupted → kept in FP16 when selected by HSWQ (**FP8:** top 5–25% by keep ratio; for SDXL and ZIT, 10% often gives sufficient quality. **ConvRot INT8 / INT8:** keep ratio **0**; FP16 set comes from automatic analysis / budget ranking, not a keep-ratio %).
...
2. **Rigorous grid / pack simulation**
   ...
   - **ConvRot INT8 / INT8:** natural absmax pack point for the symmetric INT8 grid; V4 weighted-histogram MSE ranks FP16 protection candidates (does not choose pack amax). **SDXL V3.1** then applies **FULL ConvRot** (Hadamard rotate → channelwise absmax) on remaining Linear/Conv2d, identical to `native_convert_int8_convrot.py`.
...
3. **Weighted MSE Optimization** — Finds parameters that minimize quantization error using an importance-weighted histogram (not a plain frequency histogram).
   ...
   - **V4 (SVD × RMS hybrid):** per-element importance blends **SVD structural leverage** \(L(i,j)=(U_i\cdot\sigma)^2\cdot(V_j)^2\) with **RMS magnitude**; \(\alpha\) tilts toward SVD on heavy-tailed layers. Used by Z Image / Z-Anime FP8 for amax search, and by **SDXL ConvRot INT8 / Z Image INT8** for FP16-candidate ranking at the absmax pack point. **Technical details:** [HSWQ V4 SVD-RMS — Technical Guide](md/HSWQ_V4_Hybrid_SVD_RMS_Technical_Guide.md).
...
### ConvRot INT8 (SDXL) / INT8 (Z Image)

- **Scripts:** `quantize_sdxl_hswq_v3.1.py` (SDXL ConvRot INT8; **300 MiB** FP16 budget; FULL ConvRot default ON) · `quantize_zi_int8_hswq_v1.0.py` (Z Image / NextDiT INT8; **700 MiB** FP16 budget; ZI format via `quantize_zib_hswq_v2.0.py`).
...
- **Samples:** 32 (recommended) — number of calibration samples (**same for FP8 and ConvRot INT8 / INT8**).
- **Steps:** 25 — number of inference steps per sample during calibration (**same for FP8 and ConvRot INT8 / INT8**).
- **Keep ratio (FP8):** 5–25% — keeps critical layers in FP16. For SDXL and ZIT, 10% often gives sufficient quality.
- **Keep ratio (ConvRot INT8 / INT8):** **0 (fixed)** — do not use a non-zero keep-ratio percentage for the SDXL V3.1 / Z Image V1.0 path.
