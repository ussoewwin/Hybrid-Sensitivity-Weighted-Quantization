...
- **Relation to the legacy V3.1 path:** `quantize_sdxl_hswq_v3.1.py` (static DualMonitor + V4 weighted-histogram FP16 protection under a 300 MiB budget, then FULL ConvRot on the remainder) selects protected layers from **static weight/activation statistics**. The trajectory-impact reverse hybrid supersedes it on measured cosine: the V3.1 pack scores ≈ **0.948** (25-seed cosine mean) while the reverse hybrid (`rev int620`) scores ≈ **0.9625**, with a full-INT8 reference at ≈ **0.938**.
