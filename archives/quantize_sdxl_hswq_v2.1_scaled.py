"""
SDXL FP8 quantization script (HSWQ V2.1: Scaled High-Performance Mode).
HSWQ full spec: scaling quantization (x * 448/amax). Output requires dedicated loader.
V2.1: DualMonitor 2D input support (time_embedding etc. (B, C) shape).

Features: MSE with scaling (scaled=True), save .scale key. Algorithm: Calibration & Layer Selection (V1.1, 2D support); Quantization (V2): weighted histogram, amax with scaled=True, apply scaling, cast to FP8, save .scale.
"""
import argparse
import torch
...
class DualMonitor:
    ...
    def update(self, input_tensor, output_tensor):
        with torch.no_grad():
            ...
            elif inp_detached.dim() == 2:   # Linear/embedding: (B, C) - time_embedding etc.
