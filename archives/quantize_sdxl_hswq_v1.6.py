"""
Quantize SDXL model to FP8 (HSWQ V1.6: High Precision + SageAttention2).
Sensitivity-based protection and importance-weighted optimization per HSWQ spec.
Uses unscaled mode (scaled=False), optimizes clipping threshold only for standard-loader compatibility.

V1.6 = V1.5 High Precision + SageAttention2 acceleration:
  - Bins: 8192 (High Res Histogram)
  - Candidates: 1000 (Dense Search)
  - Refinement: 10 iterations (Deep Fit)
  - NEW: --sa2 option for SageAttention2-accelerated calibration

Changelog:
- V1.5: ZIT V1.5 precision (bins=8192, candidates=1000, iterations=10)
- V1.6: SageAttention2-accelerated calibration option (--sa2)
"""
...
# === V1.6: SageAttention2 Integration ===
...
def main():
    parser = argparse.ArgumentParser(description="SDXL FP8 Quantization (HSWQ V1.6: High Precision + SageAttention2)")
    ...
    parser.add_argument("--sa2", action="store_true", help="Enable SageAttention2 for faster calibration")
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {device}")
    
    # === V1.6: SageAttention2 Initialization ===
    if args.sa2:
        if try_import_sage_attention():
            enable_sage_attention()
        else:
            print("[Warning] --sa2 specified but SageAttention2 not available.")
