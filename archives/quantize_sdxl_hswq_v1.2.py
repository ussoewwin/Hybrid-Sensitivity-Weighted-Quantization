"""
Quantize SDXL model to FP8 (HSWQ V1.2: GPU-accelerated conversion).
Implements sensitivity-based protection and importance-weighted optimization per HSWQ spec.
Uses scaled=False (no scaling) for standard-loader compatibility; optimizes clipping thresholds.

Changelog:
- V1.2: SageAttention2 option for faster calibration (--sa2)
- Current: VRAM-optimized; quantization conversion runs on GPU (previous version in archives).

Algorithm: same as V1.1/V1.2 + GPU conversion step.
"""
...
# Weighted histogram MSE optimization is in weighted_histogram_mse.py
# HSWQWeightedHistogramOptimizer performs full weighted MSE optimization
...
def main():
    ...
    for name, module in tqdm(pipeline.unet.named_modules(), desc="Analyzing"):
        if isinstance(module, (torch.nn.Conv2d, torch.nn.Linear)):
            ...
            # V16/HSWQ V1: full weighted MSE via HSWQ module
            # scaled=False: find optimal amax for compatibility (no scaling)
            # Balances avoiding over-clipping with handling outliers > 448
    ...
    # === VRAM Optimization ===
    # Free VRAM by deleting pipeline and optimizer, then move state_dict to GPU for fast clamp/cast
    ...
    for key, value in tqdm(original_state_dict.items(), desc="Converting"):
        ...
        # Store in output dict (safetensors may move to CPU on save; keep on GPU here)
    ...
    # save_file accepts GPU tensors and moves to CPU on save (safetensors). Fallback below if needed.
    try:
