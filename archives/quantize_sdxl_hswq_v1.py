"""
SDXL FP8 quantization script (HSWQ V1: Standard Compatible Mode).
HSWQ spec: Sensitivity-based protection and Importance-weighted MSE optimization.
Standard loader compatible: no scaling (scaled=False), optimize clipping threshold only.
...
Algorithm:
1. Calibration: Sensitivity (output variance) and Importance (input mean abs).
2. Layer selection: Top N% (default 25%) by sensitivity -> FP16 keep list.
3. Quantization: FP16 keep -> no quantize; others -> weighted histogram, find amax with scaled=False, clip and cast to FP8.
"""
import argparse
import torch
