"""
SDXL FP8 quantization script (HSWQ V2: Scaled High-Performance Mode).
HSWQ full spec: scaling quantization (x * 448/amax), FP8 dynamic range maximized.
Output requires dedicated loader or ComfyUI node (standard loader breaks images).

Features: MSE with scaling (scaled=True), save .scale key, better precision for small values.

Algorithm: 1. Calibration & Layer Selection (same as V1). 2. Quantization (V2): weighted histogram from Input Importance, find amax with scaled=True, apply value_scaled = value * (448.0/amax), cast to float8_e4m3fn, save inv_scale = amax/448 as .scale.
"""
import argparse
import torch
...
# --- ComfyUI-compatible mapping functions (same as V1) ---
...
def main():
    ...
    parser.add_argument("--num_calib_samples", type=int, default=25, help="Number of calibration samples (recommended: 25)")
