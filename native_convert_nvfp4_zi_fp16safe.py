"""Z-Image / ZIT NVFP4 + analysis-based FP16 protect (fp16safe variant).

NEW FILE based on native_convert_nvfp4_zi.py (do not edit the base).
Adds _FP16_PROTECT_KEYSET from moodyProMix_zitV13 weight analysis
(test/_moodyProMix_zitV13_nvfp4_fp16_protect_analysis.json):
  high abs_max FFN w2 / selected adaLN / qkv / out — kept float16 (not NVFP4).
Kitchen Turbo blacklist still kept as bfloat16 (unchanged from base).
...
Reference (pack / blacklist / metadata):
  ...
  - Analysis FP16 protect keyset (float16) before NVFP4
...
Example:
  python native_convert_nvfp4_zi_fp16safe.py --model ... --output ...
"""
...
# Analysis-based FP16 protect (moodyProMix_zitV13; Kitchen Turbo candidates only).
# Source: test/_moodyProMix_zitV13_nvfp4_fp16_protect_analysis.json
# recommended_fp16_protect_keys (union of top abs_max + high-kurtosis outliers).
_FP16_PROTECT_KEYSET: frozenset[str] = frozenset(
    {
...
def _is_fp16_protect_key(key: str) -> bool:
    """True if key is in analysis FP16 protect set (exact or prefix variants)."""
    if key in _FP16_PROTECT_KEYSET:
        return True
    if key.startswith("diffusion_model."):
        alt = "model." + key
        if alt in _FP16_PROTECT_KEYSET:
            return True
...
def convert_to_nvfp4(
    ...
    print(
        ...
        f"+ FP16 protect ({len(_FP16_PROTECT_KEYSET)} keys)"
    )
    print(f"  [FP16 protect] {len(_FP16_PROTECT_KEYSET)} analysis keys → float16")
    if enable_convrot:
        print(
    ...
    for k, v in tqdm(list(sd.items())):
        ...
        # Analysis FP16 protect (before NVFP4) — moodyProMix_zitV13 ranked set
        if _is_fp16_protect_key(k):
            new_sd[k] = v.to(dtype=torch.float16)
            n_fp16_protect += 1
            continue
    ...
    by_tag = (
        "ComfyUI Kitchen NVFP4 Converter (Z-Image ConvRot + FP16 protect)"
        if enable_convrot
        else "ComfyUI Kitchen NVFP4 Converter (Z-Image FP16 protect)"
    )
    ...
    final_metadata["hswq_fp16_protect"] = "1"
    final_metadata["hswq_fp16_protect_n"] = str(n_fp16_protect)
    final_metadata["hswq_fp16_protect_source"] = (
        "moodyProMix_zitV13_nvfp4_fp16_protect_analysis"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Z-Image / ZIT NVFP4 + analysis FP16 protect (fp16safe). "
            "Based on native_convert_nvfp4_zi.py; keeps ranked high-abs_max / "
            "high-kurtosis Linear weights as float16. FULL ConvRot ON by default. "
            "Default Kitchen profile Z-Image-Turbo."
        )
    )
    parser.add_argument(
        "--model",
