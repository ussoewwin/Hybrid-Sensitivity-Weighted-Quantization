...
Finish discipline (owner 2026-07-26 — 完走しねえコードに意味はない):
  ...
  - Every phase marker carries a CUDA + host RAM watermark (per-interval
    peak) — the peak is measured, never guessed.
  - Pack streams the source dict (pop per key): the FP16 source shrinks as
    the packed dict grows, halving pack-phase host RAM. The pack loop is
    CPU-only, so the 1500→2400 OOM was host-side, not VRAM.
...
def _mem_watermark() -> str:
    """CUDA + host RAM watermark (owner 2026-07-26: measure the peak, never
    guess). CUDA peak is per-interval — reset on every call."""
    parts = []
    if torch.cuda.is_available():
        parts.append(
            f"cuda alloc={torch.cuda.memory_allocated() / 2**20:.0f} MiB "
    ...
    try:
        ...
        parts.append(
            f"host avail={vm.available / 2**20:.0f} MiB ({vm.percent:.0f}% used)"
        )
    except Exception:
        parts.append("host n/a (psutil missing)")
    return " | ".join(parts)


def _phase_flush(msg: str) -> None:
    """Mark pipeline phase + mem watermark + flush (NO_TAIL diagnosable)."""
    print(f"{msg} [{_mem_watermark()}]", flush=True)
    sys.stdout.flush()
    sys.stderr.flush()
...
def _ensure_distribution_profile(
    *,
    ...
    Script: analyze/analyze_krea2_nvfp4_distribution.py.
    ...
    analyze_script = os.path.join(
        _script_dir(), "analyze", "analyze_krea2_nvfp4_distribution.py"
    )
...
def _load_distribution_profile_layers(
    ...
    Auto convert always re-runs analyze/analyze_krea2_nvfp4_distribution.py,
...
def resolve_veto_tunables(
    ...
    *,
    ...
    from analyze_krea2_nvfp4_distribution import (
...
def _discover_ff2_suffixes(
    ...
    """Discover FFN-out Linear suffixes (Krea2: .mlp.down) from THIS profile."""
    if not norm_profile:
        return ()
    analyze_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "analyze")
    if analyze_dir not in sys.path:
        sys.path.insert(0, analyze_dir)
    from analyze_krea2_nvfp4_distribution import _classify_layer_key
    ...
    for key in norm_profile:
        ...
        # Krea2 NextDiT: blocks.N.mlp.down  (NOT SDXL .ff.net.2)
        idx = base.rfind(".mlp.")
        if idx < 0:
            continue
        suf = base[idx:]
...
def _ff2_selective_veto_hit(
    ...
    """Selective mlp.down (ff2) VETO: class-relative profile_score and outlier."""
...
def _compute_sdxl_per_projection_attn_veto(
    ...
    (analyze_krea2_nvfp4_distribution). No additional hardcoded floors.
    """
    proj_veto = set()
...
def _collect_mse_release_candidates(
    ...
    (analyze_krea2_nvfp4_distribution). No hardcoded min/max floors.
    """
    candidates = set()
...
class DualMonitor:
    ...
    def update(self, input_tensor, output_tensor, module=None):
        with torch.no_grad():
            ...
            # Conv2d: NCHW → per-in-channel. Linear (incl. Krea2 txtfusion.projector
            # 4D [B,L,D,N]): reduce all dims except last (= in_features). Never treat
            # Linear 4D as NCHW — that maps seq length into channel stats (19 vs 23).
    ...
    if model_profile:
        for name, prof in model_profile.items():
            if isinstance(prof, dict):
                ...
                # VETO thresholds = analyze_krea2_nvfp4_distribution.derive_veto_tunables_nvfp4
...
def _ensure_comfyui_on_sys_path() -> str:
    """Locate ComfyUI root (Krea2 SingleStreamDiT lives under comfy/ldm/krea2)."""
    candidates = []
    ...
    raise FileNotFoundError(
        ...
        "Set COMFYUI_PATH or install ComfyUI next to the known paths."
    )
...
def _install_comfy_optional_stubs() -> None:
    """Lightweight stubs (same pattern as krea2_nvfp4_bench / nvfp4bench_sdxl)."""
    import types
...
def _resolve_comfy_root_for_clip(comfy_path: str | None = None) -> str:
    """Prefer --comfy_path; else same search as DiT load."""
    if comfy_path:
        root = os.path.abspath(comfy_path)
        model_py = os.path.join(root, "comfy", "ldm", "krea2", "model.py")
        if not os.path.isfile(model_py):
            raise FileNotFoundError(
                f"--comfy_path has no comfy/ldm/krea2/model.py: {root}"
            )
        if root not in sys.path:
            sys.path.insert(0, root)
        return root
    return _ensure_comfyui_on_sys_path()


def _assert_clip_safetensors_readable(clip_path: str) -> None:
    """Fail fast if --clip_path is truncated/corrupt (cloud incomplete upload).

    SafetensorError 'incomplete metadata, file not fully covered' means the
    on-disk bytes do not match the header's declared tensor span — almost
    always a partial download/copy, not a convert-logic bug.
    """
    if not os.path.isfile(clip_path):
        raise FileNotFoundError(f"--clip_path not found: {clip_path}")
    size_b = os.path.getsize(clip_path)
    if size_b < 1024:
        raise RuntimeError(
            f"--clip_path is too small to be a Qwen3-VL CLIP pack: {clip_path} "
            f"({size_b} bytes). Re-upload the full safetensors to the cloud host."
        )
    try:
        import safetensors

        with safetensors.safe_open(clip_path, framework="pt", device="cpu") as f:
            n_keys = len(list(f.keys()))
    except Exception as e:
        raise RuntimeError(
            f"--clip_path safetensors is unreadable or incomplete: {clip_path} "
            f"(size={size_b} bytes / {size_b / (1024 ** 3):.3f} GiB). "
            f"Underlying error: {type(e).__name__}: {e}. "
            "This is MetadataIncompleteBuffer / truncated file — delete the "
            "cloud copy and re-download or re-upload the full "
            "Qwen3-VL-4B CLIP .safetensors (verify byte size matches local)."
        ) from e
    if n_keys < 1:
        raise RuntimeError(
            f"--clip_path has zero tensors: {clip_path} (size={size_b} bytes)"
        )
    print(
        f"  [Krea2 calib] CLIP safetensors OK: {n_keys} tensors, "
        f"{size_b / (1024 ** 3):.3f} GiB @ {clip_path}"
    )
...
def _encode_krea2_calib_contexts(
    *,
    ...
    """
    _assert_clip_safetensors_readable(clip_path)

    saved_argv = _clear_argv_for_comfy()
    try:
        _resolve_comfy_root_for_clip(comfy_path)
...
def detect_krea2_dit_config(state_dict, key_prefix: str) -> dict:
    """Mirror comfy.model_detection Krea2 branch (no Diffusers UNet config)."""
    head_dim = 128
    ...
    block_re = re.compile(
        r"^" + re.escape(key_prefix) + r"blocks\.(\d+)\."
    )
    layers = 0
...
def load_krea2_from_safetensors(path, device="cuda"):
    ...
    for name, mod in dit.named_modules():
        ...
        if ck in state_dict:
            comfyui_to_diffusers_map[ck] = f"{name}.weight"
    print(
        f"  [Krea2] identity map entries={len(comfyui_to_diffusers_map)} "
        f"(prefix={prefix!r})"
    )
    dit.eval()
    return dit, state_dict, comfyui_to_diffusers_map


# Backward-compatible name used by run_nvfp4_calib / convert paths.
def load_unet_from_safetensors(path, device="cuda"):
    return load_krea2_from_safetensors(path, device=device)
...
def run_nvfp4_calib(
    *,
    ...
    contexts (Qwen3-VL-4B), not synthetic randn context.
    """
    ...
    # Encode CLIP contexts BEFORE DiT load (VRAM: TE then DiT, not both).
    context_bank: list[tuple[torch.Tensor, torch.Tensor | None]] | None = None
    if clip_path:
        sd_peek = load_file(input_path)
        _prefix = _find_krea2_key_prefix(sd_peek)
        _cfg = detect_krea2_dit_config(sd_peek, _prefix)
        _fused = int(_cfg["txtlayers"]) * int(_cfg["txtdim"])
        del sd_peek
        gc.collect()
        context_bank = _encode_krea2_calib_contexts(
            clip_path=clip_path,
            prompts=prompts,
            expected_fused=_fused,
        ...
        print(
            "  [WARN] --clip_path not set; DualMonitor uses synthetic "
            "randn context (not Comfy KREA2 CLIP). Prefer --clip_path "
            "with Qwen3-VL-4B for Krea2."
        )

    model, _state_dict, comfyui_to_diffusers_map = load_krea2_from_safetensors(
        input_path, device
    )
    ...
    # prompts / context_bank prepared before DiT load (see above)
    ...
    fused = int(model.txtlayers) * int(model.txtdim)
    if context_bank is not None:
        print(
            "  [Krea2 calib] real CLIP context "
            f"(channels={model.channels}, patch={model.patch}, "
            f"txtlayers={model.txtlayers}, txtdim={model.txtdim}, "
            f"fused={fused})"
        )
    else:
        # Krea2 needs Qwen3-VL multi-layer context (txtlayers * txtdim).
        # Without --clip_path: synthetic latents + context (prompt = seed only).
        print(
            "  [Krea2 calib] synthetic latent + context "
            f"(channels={model.channels}, patch={model.patch}, "
            f"txtlayers={model.txtlayers}, txtdim={model.txtdim}); "
            "prompt lines = sample seeds only"
        )
    gen = torch.Generator(device=device).manual_seed(42)
    # Latent spatial size: multiple of patch; Wan21-scale 16ch → modest H/W.
    lat_h = lat_w = 32
    ctx_seq = 256
    ...
    # SDXL upsamplers integrity N/A for Krea2; verify every mapped Linear/Conv
    # module exists on the DiT instead.
    missing_mods = sorted(
        n for n in mapped_weight_modules
        if dict(model.named_modules()).get(n) is None
    )
    if missing_mods:
        raise RuntimeError(
            f"Map integrity FATAL: {len(missing_mods)} mapped module name(s) "
            f"missing on Krea2 DiT (refuse invent)"
        )
    print(
        f"  [Map integrity] Krea2 mapped modules OK "
        f"(n={len(mapped_weight_modules)})"
    )
...
def convert_to_nvfp4_convrot(
    ...
    # Stream the source dict: pop each key so the FP16 source shrinks as the
    # packed dict grows (owner 2026-07-26 — pack-peak host RAM halves; the
    # loop below is CPU-only, so the 1500→2400 OOM was host RAM, not VRAM).
    _pack_total = len(state_dict)
    for _pack_i, key in enumerate(tqdm(list(state_dict.keys()))):
        tensor = state_dict.pop(key)
        if _pack_i and _pack_i % 256 == 0:
            _phase_flush(
                f"  [phase] pack progress keys={_pack_i}/{_pack_total}"
            )
    ...
    # Drop the streamed-empty source dict shell before write.
    del state_dict
    _cuda_empty_cache_if_available()
    _phase_flush(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "ComfyUI root with comfy/ldm/krea2 (optional). Default: "
            "COMFYUI_PATH / known install paths / repo ComfyUI-master."
        ),
    )
    parser.add_argument(
        "--keep_ratio",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(from analyze/analyze_krea2_nvfp4_distribution.py). "
    ...
    if args.clip_path:
        try:
            _assert_clip_safetensors_readable(args.clip_path)
    ...
    if abs(float(args.keep_ratio)) > 1e-12:
        print(
            ...
            "Use --fp16_budget_mb for FP16 protect."
        )
        sys.exit(1)
    try:
        _require_fp16_budget_mb_hard(float(args.fp16_budget_mb))
