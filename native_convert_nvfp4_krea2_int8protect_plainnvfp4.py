"""Krea2 plain NVFP4 + analysis ConvRot INT8 protect (plain-NVFP4 variant).

NEW FILE based on native_convert_nvfp4_krea2_int8protect.py
(do not edit that converter).

Protect path (same 20 keys):
  ConvRot rotate (W @ H^T) → row-wise INT8 + weight_scale + int8_tensorwise stamp
  in _quantization_metadata (same as ZIT int8protect).

Remaining Linear 2D: plain Kitchen NVFP4 (NO ConvRot on NVFP4 path).
Krea2 Kitchen blacklist: bfloat16 (unchanged).

Key source:
  test/_moodyKrea2Mix_v40BF16_nvfp4_int8protect20_final_keys.json

Example:
  python native_convert_nvfp4_krea2_int8protect_plainnvfp4.py --model ... --output ...
"""
...
_MODEL_TYPE = "Krea2"
# INT8 protect ConvRot preferred group (NVFP4 path does not rotate).
...
# Recommend union 10 + abs_max fill → 20 keys.
_INT8_PROTECT_KEYS_JSON = os.path.join(
    _REPO_ROOT,
    "test",
    "_moodyKrea2Mix_v40BF16_nvfp4_int8protect20_final_keys.json",
)
...
def convert_to_nvfp4(
    ...
    for k, v in tqdm(list(sd.items())):
        ...
        # Analysis ConvRot INT8 protect (before NVFP4) — 20 keys
    ...
    final_metadata["converted_by"] = (
        ...
        "(Krea2 plain NVFP4 + INT8 protect)"
    )
    ...
    del new_sd
    del quant_map
    _release_vram("after native Krea2 plain-NVFP4 int8protect convert save")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Krea2 plain NVFP4 + analysis ConvRot INT8 protect. "
            "Based on native_convert_nvfp4_krea2_int8protect.py; "
            "20 ranked Linear as ConvRot INT8; rest plain NVFP4 (no rotate)."
        )
    )
    parser.add_argument(
        "--model",
