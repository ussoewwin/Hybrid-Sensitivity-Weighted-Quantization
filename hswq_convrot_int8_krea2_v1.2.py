...
DualMonitor calib (--calib_file + --clip_path, CLIPType.KREA2):
  ...
  Runs when keep_sensitive>0 OR bias_correction OR BOTH paths are set.
  keep_sensitive exists to use DualMonitor — it FORCES run_card1_calib
...
Reverse protect (--keep_sensitive N):
  DualMonitor consumer — forces DualMonitor / run_card1_calib.
  DualMonitor axis (UNCHANGED formula): activation-weighted
  ||W-W_q||/||W|| from E[x^2] only — same math as before.
  Complement axis: weighted histogram MSE (weighted_histogram_mse_fast;
  SDXL V1.3 origin; logic transplanted here, not imported from
  quantize_sdxl_hswq_v1.3.py).
  Both axes → pool midrank → auto IQR/median weights → weighted
  geometric mean composite. Higher composite = more protect priority.
  Works with bias OFF. No Frobenius escape / no Static Profile VETO / no SDXL.
"""
...
from weighted_histogram_mse_fast import (  # noqa: E402
...
def _histogram_mse_score(
    ...
    """V1.3-style weighted histogram MSE at optimal amax (scaled=False).
...
def convert_to_int8(
    ...
    *,
    ...
    run_dual_monitor = use_keep_sensitive or use_bias or have_calib_paths

    if run_dual_monitor and not have_calib_paths:
        raise ValueError(
            "DualMonitor requires --calib_file and --clip_path "
            "(keep_sensitive forces DualMonitor; bias and path-only "
    ...
    if run_dual_monitor:
        ...
        if use_keep_sensitive:
            print(
                ...
                f"HistMSE midrank | top-N={int(keep_sensitive)} → original dtype"
            )
    ...
    if use_keep_sensitive:
        ...
        print(
            f"  [HistMSE] Complement axis ON | "
            f"weighted_histogram_mse_fast on {hist_dev} "
            f"(DualMonitor formula unchanged)"
        )
    ...
    for key, tensor in tqdm(list(state_dict.items())):
        ...
        elif use_keep_sensitive:
            # keep_sensitive ranks DualMonitor E[x^2] only — no Frobenius escape.
            pass
        else:
        ...
        if use_keep_sensitive and hist_opt is not None:
            imp = (
                importance_dict.get(module_name_sens)
                if module_name_sens is not None
                else None
            )
            try:
                ...
                # Skip hist axis for this layer; composite handles missing entries.
                pass
    ...
    # --- Sensitivity-based revert: DualMonitor × HistMSE midrank composite ---
    ...
    if use_keep_sensitive and not layer_hist_mse:
        raise RuntimeError(
            "keep_sensitive DualMonitor x HistMSE composite requires the "
            "HistMSE axis, but no layer produced a weighted_histogram_mse_fast "
            "score (check hist_opt init / weighted_histogram_mse_fast import)"
        )

    if keep_sensitive > 0 and layer_hist_mse:
        ...
        if dm_real_keys:
            ...
            # No real DM scores — degenerate to pure HistMSE ranking.
            weights = {
        ...
        for i, k in enumerate(pool_keys):
            r_dm = dm_rank_lookup.get(k, 0.5)  # neutral midrank for DM-unknown
    ...
    if int(keep_sensitive) > 0:
        print(
            "  Ranking: DualMonitor E[x^2] (unchanged) × HistMSE "
            "pool-midrank weighted geometric (auto IQR/median weights)"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "--keep_sensitive N forces DualMonitor calib (needs --calib_file + "
            "--clip_path). Paths alone also calibrate. Card 1 = --bias_correction "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "DualMonitor mu_x / E[x^2] feed Card 1 and keep_sensitive."
        ),
    )
    parser.add_argument(
        "--clip_path",
        ...
        help=(
            ...
            "(bias OFF still calib; keep_sensitive consumes DualMonitor E[x^2])."
        ),
    )
    parser.add_argument(
        "--comfy_path",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--clip_path). Keep top N layers ranked by DualMonitor E[x^2]-weighted "
            "||W-W_q||/||W|| (formula unchanged) COMPOSITED with weighted "
            "histogram MSE via pool midrank + auto IQR/median weights. "
