...
def _release_vram_before_bench(label: str = "post-convert") -> None:
    ...
    Convert / Card-1 calib can leave GPU tensors until refs are deleted and the
    allocator cache is flushed. Without this clear, the chained bench child
    OOMs on the same GPU.
    """
...
def convert_to_int8(
    ...
    # Convert complete: drop holdings before optional chained bench.
    del new_state_dict
    del state_dict
    ...
    comfyui_to_diffusers_map.clear()
    del comfyui_to_diffusers_map
    if h_matrix is not None:
        del h_matrix
    _release_vram_before_bench("after INT8 convert save")
