...
Reuses benchmark/zi_int8_bench.py for ComfyUI bootstrap / model loading / prompt encoding.
...
BENCH = os.path.join(_BENCH_DIR, "zi_int8_bench.py")
_spec = importlib.util.spec_from_file_location("zi_bench", BENCH)
...
def main() -> int:
    ...
    try:
        ...
        int8 = bench._load_diffusion_model(args.fp8)
        int8_runs = {}
        for s in seeds:
            print(f"[INT8] seed {s}")
            xs, x0s, final = run_trajectory(
