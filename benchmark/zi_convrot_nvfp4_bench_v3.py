...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Z Image ConvRot NVFP4 Fidelity & VRAM Benchmark - TC (W4A4) mode "
            "(ComfyUI standard pipeline; --tc forces the HSWQ TensorCore forward)"
        )
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Force the HSWQ TensorCore (W4A4) forward for the NVFP4 path "
            "(overrides auto-detect; requires a calibrated *_calib.safetensors "
            "with .input_scale keys, otherwise SSIM collapses)."
        ),
    )
    parser.add_argument(
        "--cfg",
    ...
    if args.tc:
        ...
        print("  [TC mode] --tc ON: forcing HSWQ TensorCore (W4A4) forward")
    if bench_use_native:
    ...
    try:
        ...
        mode = "TC W4A4" if args.tc else "auto (TC if calibrated, else parity)"
        print(
            f"  [BENCH] SSIM target >={SSIM_TARGET} "
            f"(ComfyUI standard pipeline + ConvRot NVFP4; path={mode})"
        )
