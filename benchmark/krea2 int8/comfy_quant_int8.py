...
def _patch_ops_decode_and_conv() -> bool:
    ...
    if not getattr(original_load, "_hswq_int8_decode_patched", False):
        ...
        def _load_quantized_module_patched(
            ...
            qfmt = str(getattr(module, "quant_format", ""))
            layout_name = str(getattr(module, "layout_type", getattr(w_params, "__class__", "")))
            if qfmt == "int8_tensorwise" or "int8" in layout_name.lower() or "tensorwiseint8" in type(w_params).__name__.lower():
                module._hswq_int8 = True
            return res
