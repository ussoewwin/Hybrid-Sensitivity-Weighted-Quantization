...
ConvRot (optional, --convrot):
  Offline Hadamard rotation of weights (W_rot = W @ H^T, group-wise).
  Stamps comfy_quant with convrot=true + convrot_groupsize.
  At runtime the HSWQ forward path applies online act rotation
  (rotate_last_dim_fast: O(N log N) float32 butterfly, zero accumulation
  error) so (x @ H) @ (W @ H^T)^T == x @ W^T.
  Combined stack: SmoothQuant (channel balance) → ConvRot (decorrelation)
  → NVFP4 (quantize the well-conditioned result).
...
def convert_to_nvfp4(
    ...
    use_convrot = convrot and build_hadamard is not None
    if convrot and build_hadamard is None:
        print("[WARN] --convrot requested but native_convert_int8 not found; ConvRot disabled.")
    modes = []
    if use_convrot: modes.append("ConvRot")
    if use_smoothquant: modes.append("SmoothQuant")
        ...
        if v.ndim == 2 and ".weight" in k:
            ...
            if use_smoothquant:
                ...
                if act_sq is not None and act_sq.shape[0] == v_tensor.shape[1]:
                    ...
                    # Keep fp32 for ConvRot (next step); cast to bf16 only right before quantize
                    v_tensor = v_f32
                    sq_applied = True
                    n_smoothquant += 1
                else:
            ...
            # W_rot = W @ H^T (group-wise). Runtime applies x @ H to match.
            ...
            if use_convrot:
                ...
                if gs is not None:
                    # Rotate in float32 for precision, then we cast later
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--convrot",
        action="store_true",
        help="Apply offline Hadamard rotation (ConvRot) to weights before NVFP4 quantize. "
             "Runtime applies matching online act rotation (rotate_last_dim_fast). "
             "Requires native_convert_int8.py in the same directory.",
    )
    parser.add_argument(
        "--convrot_groupsize",
        type=int,
