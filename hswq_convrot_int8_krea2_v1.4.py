...
Data-driven reverse ranking (on top of fixed blacklist):
  --blacklist_keep N: top N highest-error DiT weights (from DualMonitor x
    HistMSE composite ranking) -> original dtype. Applied AFTER fixed
...
def convert_to_int8(
    ...
    *,
    ...
    if use_reverse_rank and not layer_hist_mse:
        raise RuntimeError(
            "blacklist_keep / keep_sensitive DualMonitor x HistMSE composite "
            "requires the HistMSE axis, but no layer produced a "
            "weighted_histogram_mse_fast score (check hist_opt init / "
            "weighted_histogram_mse_fast import)"
        )
